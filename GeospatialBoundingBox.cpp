...
Vector3f GeospatialBoundingBox::getCentroid()	{
	return centroid;
}
...
void GeospatialBoundingBox::initialize(Image *resampled)	{
	...
	if (!resampled)	{
		...
		Image *hole_filled_map = new Image();
		hole_filled_map->copy(xyz_map);
		ImageProcessing::holeFill(hole_filled_map,Color(0.0f),6,2);
		hole_filled_map->saveImage(_format("%s_B.pfm",file_name.c_str()));
		std::cout << _format("XYZ map %s_B.pfm saved.",file_name.c_str()) << std::endl;

		///Perform point reduction by removing extra points added by the hole filling
		ImageProcessing::pointReduction(xyz_map, hole_filled_map, 10);
		xyz_map->copy(hole_filled_map);
		exportXYZMap("C");
		delete hole_filled_map;
		...
		Image *filtered_image = ImageProcessing::bilateralFilteringGrayscale(zzz_map,0.5,0.35,0.1,0.05);
...
bool GeospatialBoundingBox::liesInside(Vector3f const &point)	{
	return ( (point(0) + GEO_EPSILON >= min_pt(0)) && (point(0) <= max_pt(0) + GEO_EPSILON) &&
			 (point(1) + GEO_EPSILON >= min_pt(1)) && (point(1) <= max_pt(1) + GEO_EPSILON) &&
			 (point(2) + GEO_EPSILON >= min_pt(2)) && (point(2) <= max_pt(2) + GEO_EPSILON) );
}
...
bool GeospatialBoundingBox::resample(double sampling_tolerance, double step)	{
	...
	while(size_not_ok)  {
		size_not_ok = false;
		for (int i=0;i<sorted_vertices_based_x.size();i++)     {
			int starting_pos = i;
...
void GeospatialBoundingBox::computeHeightAndNormalVariation()	{
	...
	for (int y=0;y<xyz_map->getHeight();y++)	{
		for (int x=0;x<xyz_map->getWidth();x++)	{
			...
			if (max_minus_min < GEO_EPSILON)    max_minus_min = 1.0f;
