...
class Image	{
	public:
		///Constructor
		Image(bool _verbose = false);
		Image(Image const &img, bool _verbose = false);
		Image(int _width, int _height, bool _verbose = false);
		Image(int _width, int _height, float r, float g, float b, float a, bool _verbose = false);
		...
		///checks if the pixel +- buffer is on the image boundary
		friend bool onImageBoundary(Image *map, Vector2i const &index, int buffer=1);
		...
		///Subimage
		Image *subimage(Vector2i const &top_left, Vector2i const &bottom_right);

        ///Focal length
        float getFocalLengthMillimeters();
		...
		float *getData3Channels();
        float *getData1Channel(int index);
	...
	private:
		...
		///Flag to determine if tables have already been computed
		static bool already_computed;
		...
		///Image loader for PFM
		bool loadPFM(std::string const &file_name);

		///Image exporter for PFM
